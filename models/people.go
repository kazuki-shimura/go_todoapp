...
// People全取得
func GetAllPeople() (peoples []People, err error) {
	cmd := `select id, uuid, firstname, lastname, email,
		password, created_at, updated_at from people`
	...
	for rows.Next() {
		var people People
		err = rows.Scan(&people.ID, &people.UUID, &people.Firstname, &people.Lastname,
			&people.Email, &people.Password, &people.CreatedAt, &people.UpdatedAt)
		if err != nil {
			log.Fatalln(err)
		}
